...
export async function followOrUnfollowUser({username,shouldFollow}:FollowUserMuation) {
    ...
    try {
        ...
        if (response.status === 204) {
            console.log(`You have ${shouldFollow ? "followed" : "unfollowed"} ${username}.`);
            return `You have ${shouldFollow ? "followed" : "unfollowed"} ${username}.`
        } else {
