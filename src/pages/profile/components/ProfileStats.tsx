  ...
  return (
    <div className="w-full h-full flex flex-wrap items-center justify-center gap-3">
      {profile?.repositories?.totalCount && (
        <ProfileStatsCard
          figure={profile.repositories.totalCount}
          title="Repositories"
        />
      )}
      {profile?.followers?.totalCount && (
        <ProfileStatsCard
          figure={profile.followers.totalCount}
          title="Followers"
        />
      )}
      {profile?.following?.totalCount && (
        <ProfileStatsCard
          figure={profile.following.totalCount}
          title="Following"
        />
      )}
      {profile?.watching?.totalCount && (
        <ProfileStatsCard
          figure={profile.watching.totalCount}
          title="Watching"
        />
      )}
      {profile?.gists?.totalCount && (
        <ProfileStatsCard figure={profile.gists.totalCount} title="Gists" />
      )}
      {profile?.starredRepositories?.totalCount && (
        <ProfileStatsCard
          figure={profile.starredRepositories.totalCount}
          title="Starred"
        />
      )}
    </div>
